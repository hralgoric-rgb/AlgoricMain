...
import { Heart, Star, Building, Search, AlertTriangle, Loader2, Award, MapPin, Calendar, Briefcase } from 'lucide-react';
...
export default function BuildersPage() {
  ...
  useEffect(() => {
    ...
    if (activeFilter === 'top-rated') {
      ...
      filtered = filtered.filter(builder => isFavorite(builder._id, 'builders'));
    }
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(
        <Star key={`full-${i}`} size={16} fill="#FFD700" stroke="#FFD700" className="text-yellow-500" />
      );
    }
    ...
    if (hasHalfStar) {
      stars.push(
        <Star key="half" size={16} fill="#FFD700" stroke="#FFD700" className="text-yellow-500" />
      );
    }
    ...
    for (let i = 0; i < emptyStars; i++) {
      stars.push(
        <Star key={`empty-${i}`} size={16} stroke="#6B7280" className="text-gray-400" />
      );
    }
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
          ...
          >
            <div className="inline-block bg-gradient-to-r from-orange-500 to-orange-600 px-6 py-2 rounded-full mb-6">
              <span className="text-white font-medium tracking-wider text-sm">PREMIUM BUILDERS</span>
            </div>

            <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold text-white leading-tight mb-8">
              Find Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-600">Dream Home</span> Builder
            </h1>

            <p className="text-xl text-gray-300 mb-10 max-w-2xl">
              Connect with India`&apos;`s most prestigious builders to create the home you`&apos;`ve always envisioned.
            </p>
            ...
            >
              <a href="#builders-section" className="inline-flex items-center gap-2 px-8 py-3 bg-orange-500 text-white font-medium rounded-full hover:bg-orange-600 transition-all duration-300 shadow-lg shadow-orange-500/20">
                <Building size={18} />
                Explore Builders
              </a>
              <a href="/contact" className="inline-flex items-center gap-2 px-8 py-3 bg-transparent border border-white/30 text-white font-medium rounded-full hover:bg-white/10 transition-all duration-300">
                <MapPin size={18} />
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-12">
            ...
            >
              ...
              <p className="text-gray-400">Discover the finest builders in the industry</p>
            </motion.div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-neutral-900/50 rounded-2xl border border-orange-900/20">
              <AlertTriangle size={40} className="mx-auto text-orange-500/50 mb-4" />
              <h3 className="text-xl font-semibold text-orange-500 mb-2">{error}</h3>
              <button
                ...
                >
                  <Link href={`/builders/${builder._id}`}>
                    <div className="relative bg-neutral-900 rounded-2xl overflow-hidden shadow-lg hover:shadow-orange-500/20 transition-all duration-300 border border-neutral-800/50">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-1 mt-1">
                                ...
                                <span className="text-white text-xs ml-1">({builder.rating})</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      {/* Card Content */}
                      <div className="p-6">
                        <p className="text-gray-400 text-sm mb-5 line-clamp-2">{builder.description}</p>
                        
                        {/* Stats */}
                        <div className="grid grid-cols-3 gap-2 mb-5">
                          <div className="flex flex-col items-center bg-neutral-800/50 rounded-lg py-2">
                            <span className="text-orange-500 font-semibold">{builder.projects}</span>
                            <span className="text-xs text-gray-400">Projects</span>
                          </div>
                          <div className="flex flex-col items-center bg-neutral-800/50 rounded-lg py-2">
                            <span className="text-orange-500 font-semibold">{builder.completed}</span>
                            <span className="text-xs text-gray-400">Completed</span>
                          </div>
                          <div className="flex flex-col items-center bg-neutral-800/50 rounded-lg py-2">
                            <span className="text-orange-500 font-semibold">{builder.ongoing}</span>
                        ...
                        <div className="space-y-2 mb-5">
                          <div className="flex items-center gap-2 text-sm">
                            ...
                            <span className="text-gray-400">Est. {builder.established}</span>
                          </div>
                          <div className="flex items-center gap-2 text-sm">
                            <Briefcase size={14} className="text-orange-500" />
                            <span className="text-gray-300">{builder.specialization}</span>
                          </div>
                        </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-neutral-800">
                          <div className="relative overflow-hidden group-hover:overflow-visible">
                            <div className="flex justify-between items-center">
                              ...
                              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-orange-500 transform transition-transform duration-300 group-hover:translate-x-1">
                                <path d="M5 12h14"/>
          ...
          {!isLoading && !error && filteredBuilders.length === 0 && (
            <div className="text-center py-16 bg-neutral-900/50 rounded-2xl border border-orange-900/20">
              ...
              <h3 className="text-xl font-semibold text-orange-500 mb-2">No builders found</h3>
              <p className="text-gray-400 max-w-md mx-auto">We couldn`&apos;`t find any builders matching your search criteria. Try adjusting your search or explore our full list of builders.</p>
              <button
                onClick={() => {
                  setSearchTerm('');
                  setActiveFilter('all');
          ...
          <div className="max-w-4xl mx-auto">
            ...
            >
              <div className="inline-block bg-white/20 backdrop-blur-sm px-6 py-2 rounded-full mb-6">
                <span className="text-white font-medium tracking-wider text-sm">JOIN OUR NETWORK</span>
              </div>
              <h2 className="text-4xl sm:text-5xl font-bold mb-6 text-white">Are You a Premium Builder?</h2>
              <p className="text-lg text-white/80 mb-8 max-w-2xl mx-auto">
                Partner with 100Gaj to showcase your projects to our extensive network of property seekers and increase your visibility in the luxury real estate market.
              </p>
            </motion.div>
            ...
            >
              <div className="bg-black/30 backdrop-blur-sm p-6 rounded-xl text-center">
                <p className="text-3xl sm:text-4xl font-bold text-white mb-2">2.5M+</p>
                <p className="text-sm text-white/70">Monthly Visitors</p>
              </div>
              <div className="bg-black/30 backdrop-blur-sm p-6 rounded-xl text-center">
                <p className="text-3xl sm:text-4xl font-bold text-white mb-2">150+</p>
                <p className="text-sm text-white/70">Premium Builders</p>
              </div>
              <div className="bg-black/30 backdrop-blur-sm p-6 rounded-xl text-center">
                <p className="text-3xl sm:text-4xl font-bold text-white mb-2">5000+</p>
